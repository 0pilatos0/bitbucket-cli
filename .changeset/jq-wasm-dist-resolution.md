...
- The CLI now exits deterministically after a command completes: on Windows, handles left behind by jq-wasm's Emscripten runtime (or an HTTP keep-alive socket) could keep the process alive forever after `--jq` had already printed its output
