    ...
    async (error: AxiosError) => {
      ...
      if (error.response) {
        ...
        throw new BBError({
          ...
          message: isTimeout
            ? `Network error: Request to Bitbucket API timed out after ${error.config?.timeout ?? instance.defaults.timeout}ms. The server accepted the connection but did not respond in time. Increase or disable the timeout via BB_HTTP_TIMEOUT (milliseconds; set BB_HTTP_TIMEOUT=0 to disable), or run with DEBUG=true for details.`
