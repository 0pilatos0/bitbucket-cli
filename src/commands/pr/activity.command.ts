    ...
    if (invalid.length > 0) {
      throw new BBError({
        ...
        message:
          ...
          // A comma list can have several bad tokens; suggest per token.
          // `didYouMeanSuffix` returns '' when there is nothing to suggest.
          invalid
            .map((type) => didYouMeanSuffix(type, VALID_ACTIVITY_TYPES))
            .join(''),
        context: { invalid },
      });
    }
