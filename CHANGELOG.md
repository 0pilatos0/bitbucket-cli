...
- [#327](https://github.com/0pilatos0/bitbucket-cli/pull/327) [`156bb86`](https://github.com/0pilatos0/bitbucket-cli/commit/156bb862872786f0489885fa8a666022eee9dea5) Thanks [@0pilatos0](https://github.com/0pilatos0)! - `bb api` now surfaces the full upstream error exchange (issue [#323](https://github.com/0pilatos0/bitbucket-cli/issues/323)):
...
- [#302](https://github.com/0pilatos0/bitbucket-cli/pull/302) [`7b4e60c`](https://github.com/0pilatos0/bitbucket-cli/commit/7b4e60c4b8d52ab8d8ee10b1b6a7fbd18fff8996) Thanks [@0pilatos0](https://github.com/0pilatos0)! - Add "did you mean" suggestions for typos and actionable next steps on auth/permission/not-found failures.
  ...
  **Suggestions.** A close-but-wrong command name or option value now gets a correction:
  ...
  - Every `must be one of` enum value (`--state`, `--kind`, `--priority`, `--sort`, `--status`, `--role`, `--strategy`, `--color`), plus `bb config get/set <key>`, `bb pr activity --type` (per bad token in a comma list, deduped), and both HTTP-method forms of `bb api` — `-X/--method` and the leading positional (`bb api GTE /user`), which are validated in different places and would otherwise answer the same typo differently.
  - Matching folds case, so `--state opne` suggests `OPEN` even though the allowed values are uppercase. Enum values are still _accepted_ case-sensitively, and a value that is right apart from its case gets its own message — `(Values are case-sensitive — use OPEN.)` — rather than the user's own word echoed back.
...
- [#300](https://github.com/0pilatos0/bitbucket-cli/pull/300) [`24c85d7`](https://github.com/0pilatos0/bitbucket-cli/commit/24c85d7ecf799679c934fc15e896ba91fa5c38f8) Thanks [@0pilatos0](https://github.com/0pilatos0)! - Update runtime and tooling dependencies: axios 1.19, chalk 6, commander 15, and jq-wasm 3 (which cuts the bundled CLI from ~3.0 MB to ~1.8 MB). No behavioural or API changes.

## 1.22.0

### Minor Changes

- [#299](https://github.com/0pilatos0/bitbucket-cli/pull/299) [`d838063`](https://github.com/0pilatos0/bitbucket-cli/commit/d838063b6d50e33c9e5756aa62730452effe3fed) Thanks [@0pilatos0](https://github.com/0pilatos0)! - `bb pr comments list` now shows a `Status` column (`resolved`, `pending`, or `open`) and supports `--resolved` / `--unresolved` flags to filter comments by resolution state, closing the remaining part of [#292](https://github.com/0pilatos0/bitbucket-cli/issues/292). The active filter is echoed under `filters.resolution` in `--json` output.
