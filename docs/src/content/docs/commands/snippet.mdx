...
:::danger
Deletion is permanent and cannot be undone. In an interactive terminal the command asks `Continue? (y/N)` when `--yes` is missing. Anywhere else (pipes, CI, `--json`, [`--no-input`](/reference/global-flags/#--no-input)) it exits with `This will permanently delete snippet <id>.` followed by `Use --yes to confirm.`
:::
...
- Without `--yes` the command asks `Continue? (y/N)` in an interactive
  terminal. Anywhere else (see [`--no-input`](/reference/global-flags/#--no-input)) it fails with
