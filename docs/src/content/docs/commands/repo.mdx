...
`--json` emits `{ success, workspace, repoSlug, uploaded }`, where `uploaded` lists the artifact names. A missing file, or two files sharing a basename, fails before anything is sent.

The whole upload must finish within the HTTP timeout (30 seconds by default). For large artifacts or slow links, raise it with [`BB_HTTP_TIMEOUT`](/reference/environment-variables/), for example `BB_HTTP_TIMEOUT=600000 bb repo downloads upload big.zip`.
