...
Delete a comment from a pull request. Asks for confirmation unless `--yes` is passed.
...
- Without `--yes`, `bb pr comments delete` asks `Continue? (y/N)` in an interactive terminal. Anywhere else (see [`--no-input`](/reference/global-flags/#--no-input)) it exits with a validation error ending in `Use --yes to confirm.`
