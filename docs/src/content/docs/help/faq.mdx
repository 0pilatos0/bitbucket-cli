...
- `bb completion install` asks which shell you use.

Scripts and CI are unaffected. Prompts need stdin and stdout to both be terminals, and `--json`, `--no-input` or `BB_PROMPT_DISABLED=1` turn them off. Without prompts a destructive command fails with `Use --yes to confirm.`, so pass the flag:
