...
**OAuth login requests one fixed set:** `account`, `repository`, `repository:admin`, `pullrequest`, `pullrequest:write`. You cannot change it. That covers auth, repositories, and pull requests, but **not** `bb repo delete`, `bb pipeline *`, `bb snippet *`, `bb project *`, or `bb webhook *` — for those, log in with an API token carrying the scopes below.
