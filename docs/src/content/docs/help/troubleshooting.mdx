...
BB_DEBUG=http bb repo list                 # [HTTP] tracing on stdout, secrets redacted
```
...
### Destructive commands fail instead of asking
...
These commands only ask `Continue? (y/N)` in an interactive terminal. You get
this error instead when stdin or stdout is piped or redirected, under `--json`
or `--no-input`, or when `BB_PROMPT_DISABLED` is set. This affects
`bb repo delete`, `bb repo default-reviewers remove`, `bb repo downloads delete`,
`bb pr comments delete`, `bb snippet delete`, `bb snippet comments delete`,
`bb webhook delete`, `bb branch-restriction delete`, `bb ssh-key delete`, and
`bb gpg-key delete`. Pass the flag:
