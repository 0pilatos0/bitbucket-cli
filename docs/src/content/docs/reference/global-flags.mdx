...
| `--no-input` | boolean | _off_ | Never prompt, even in an interactive terminal (except `bb completion install`, see [`--no-input`](#--no-input)). Also honoured: `BB_PROMPT_DISABLED`. |
...
A few commands ask for what is missing when you run them in an interactive
terminal:

- Destructive commands (`bb repo delete`, `bb repo default-reviewers remove`,
  `bb repo downloads delete`, `bb pr comments delete`, `bb snippet delete`,
  `bb snippet comments delete`, `bb webhook delete`,
  `bb branch-restriction delete`, `bb ssh-key delete`, `bb gpg-key delete`)
  name what they are about to change and ask `Continue? (y/N)` instead of
  failing without `--yes`.
- `bb pr create` asks for the title, and then an optional description, when
  `--title` is missing.
- `bb auth login` asks whether to use the browser (OAuth) or an API token when
  no auth flag is given, and asks for a missing username or token.

Prompts only appear when stdin **and** stdout are both terminals. Piped input,
redirected output, `--json`, `--no-input` and a non-empty `BB_PROMPT_DISABLED`
each switch them off, and the command then behaves exactly as it does in a
script: a missing `--yes` or `--title` is an error, and `bb auth login` with no
flags goes straight to OAuth.

`bb completion install` is the exception. Its shell question comes from the
completion installer and ignores all of these switches; see
[`bb completion install`](/commands/completion/#bb-completion-install).

```bash
bb repo delete myworkspace/old-repo            # asks Continue? (y/N)
bb repo delete myworkspace/old-repo --yes      # never asks
bb repo delete myworkspace/old-repo --no-input # fails: Use --yes to confirm.
```

Answering anything other than `y` or `yes`, or pressing Ctrl+C or Ctrl+D at a
prompt, stops the command with error
[5004 PROMPT_CANCELLED](/reference/error-codes/#5004---prompt_cancelled) and
nothing is changed. Ctrl+C exits with status 130, the rest with 1.
...
Both flags are available on these eighteen commands:
