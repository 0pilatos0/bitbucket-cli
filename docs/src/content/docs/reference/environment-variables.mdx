...
| `BB_PROMPT_DISABLED` | When set to any non-empty value, never prompt, even in an interactive terminal. Same effect as the global `--no-input` flag. See [`--no-input`](/reference/global-flags/#--no-input). | `1` |
