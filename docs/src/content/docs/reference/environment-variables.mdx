...
| `BB_DEBUG` | HTTP debug tracing on stdout. `http` logs method, URL, status (or network error code) and elapsed time for every API call; `verbose` also logs the response bodies, secrets redacted. Any other non-empty value (e.g. `off`) disables tracing, even when `DEBUG=true` is set. See [HTTP debug tracing](#http-debug-tracing). | `http` |
