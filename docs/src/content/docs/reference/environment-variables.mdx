...
`(attempt N)`. The elapsed time covers one attempt on the wire, not the
backoff wait before it. A request that gets no response logs the network error
code instead of a status, e.g. `ECONNRESET` or `ECONNABORTED` for a timeout.

`BB_DEBUG=verbose` adds a `Response Body:` (or `Error Response Body:`) line
after each outcome. Query strings are always replaced with `?[redacted]`, and
values under keys such as `access_token`, `password` and `authorization` are
replaced with `[REDACTED]` in bodies. Request bodies and headers are never
logged.
