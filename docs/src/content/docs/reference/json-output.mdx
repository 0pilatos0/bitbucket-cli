...
- If the output is an envelope object, the CLI looks for the first of these
  ...
  entries       downloads     results   webhooks  values
  ```
...
| `repoSlug` | string | Repository-scoped collections: `pr list`, `pr comments list`, `pr activity`, `pipeline list`, `commit list`, `status list`, `repo ls`, `repo downloads list`, `webhook list` (default `--scope repo` only) |
