...
- If the output is an envelope object, the CLI looks for the first of these
  ...
  results       webhooks      branchRestrictions  sshKeys  gpgKeys
  deployments   environments  values
  ```
...
| `repoSlug` | string | Repository-scoped collections: `pr list`, `pr comments list`, `pr activity`, `pipeline list`, `commit list`, `status list`, `webhook list` (default `--scope repo` only), `branch-restriction list`, `deployment list`, `deployment environments` |
