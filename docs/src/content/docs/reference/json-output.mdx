...
- If the output is an envelope object, the CLI looks for the first of these
  ...
  entries       downloads     results   webhooks  branchRestrictions
  sshKeys       gpgKeys       deployments         environments  values
  ```
...
`status list`, `workspace list`, `project list`, `search code`, `webhook list`
...
|-------|------|-----------|
| `workspace` | string | All collections except `workspace list` (whose slugs live in the items) and the account-scoped `ssh-key list` / `gpg-key list` |
| `repoSlug` | string | Repository-scoped collections: `pr list`, `pr comments list`, `pr activity`, `pipeline list`, `commit list`, `status list`, `repo ls`, `repo downloads list`, `webhook list` (default `--scope repo` only), `branch-restriction list`, `deployment list`, `deployment environments` |
| `count` | number | All collections |
| `state` | string | `pr list` |
| `filters` | object | `pr list`, `pr activity`, `pr comments list`, `branch-restriction list` — always present, with an unset value inside (`{"mine": false}`, `{"types": []}`, `{"resolution": null}`) or `{}` |
| `commit` | string | `status list` |
| `sort` | string | `pipeline list` |
| `ref`, `path` | string | `repo ls` |
| `query` | string | `search code` — the query as sent, including any `repo:` scope added by `--repo` |
...
| `webhook view`, `webhook create` | `{ workspace, repoSlug, webhook }` (no `repoSlug` with `--scope workspace`) |
...
Used by: `pr approve`, `pr decline`, `pr merge`, `pr ready`, `pr comments add/edit/delete/reply/resolve/unresolve`, `pr reviewers add/remove`, `webhook delete`, `auth logout`
...
`webhook delete` returns `{ "success": true, "workspace": "...", "repoSlug": "...", "webhookId": "{...}" }`
(no `repoSlug` with `--scope workspace`).
...
| `response` | object \| string | `APIError` only, and only when the upstream response had a body. The raw Bitbucket error payload — usually a JSON object, but a plain-text body (e.g. a request-parser `Bad Request`) is passed through as a string. |
| `headers` | object | `bb api` only. The upstream response headers, so scripts can inspect rate-limit or request-id headers on failure. |
| `statusText` | string | `bb api` only, and only when the transport provides a reason phrase (HTTP/2 omits it). |
