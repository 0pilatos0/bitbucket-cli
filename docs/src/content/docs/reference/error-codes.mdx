...
carry Bitbucket's own explanation. When Bitbucket returns a bare plain-text body
(e.g. `Bad Request` from the request parser before the endpoint runs), the body
text becomes `message` instead of axios's generic "Request failed with status
code 400".
...
`bb api` goes further: its `--json` error object also includes `headers` and the
transport's `statusText`, and `bb api -i` prints the status line and headers to
stderr on failure. See [API Command](/commands/api/#notes).
...
### 5004 - PROMPT_CANCELLED

**Message:** `Cancelled.` (a declined confirmation) or `Prompt cancelled.`
(Ctrl+C or Ctrl+D at any prompt)

**Cause:** You answered no at a `Continue? (y/N)` confirmation, or interrupted
an interactive prompt. Prompts only appear in an interactive terminal, so
scripts and CI never see this code. See
[`--no-input`](/reference/global-flags/#--no-input).

The process exits with status 1, except after Ctrl+C, which exits with 130
like any other interrupted command.
...
> Network error: Request to Bitbucket API timed out after 30000ms. The server accepted the connection but did not respond in time. Increase or disable the timeout via BB_HTTP_TIMEOUT (milliseconds; set BB_HTTP_TIMEOUT=0 to disable), or run with BB_DEBUG=http for details.
...
```bash
BB_DEBUG=verbose bb <your-command> 2>&1
```
