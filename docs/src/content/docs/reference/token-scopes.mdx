...
That covers repository, pull request, commit, build-status, workspace and
branch-restriction commands, plus `bb ssh-key list` and `bb gpg-key list`. It
does **not** cover `bb pipeline`, `bb deployment`, `bb snippet`,
`bb project`, `bb webhook`, `bb repo delete`, or adding and deleting SSH/GPG
keys. Use an API token for those.
</Aside>
...
| Scope | Grants |
|-------|--------|
| `read:user:bitbucket` | Read user profiles — verifies any login, resolves your UUID for `bb pr list --mine` and the `bb ssh-key` / `bb gpg-key` commands, and covers `bb workspace list` |
| `read:repository:bitbucket` | List and view repositories, commits, and commit build statuses; search code |
...
for the workspace. See [Search Commands](/commands/search/).
