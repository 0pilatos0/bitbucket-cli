...
The CLI requests a hardcoded OAuth scope set: `account repository repository:admin pullrequest pullrequest:write`. A custom consumer cannot widen it. `bb pipeline`, `bb snippet`, `bb project` and `bb repo delete` are not covered — use an API token for those. See [Token Scopes](/reference/token-scopes/).
</Aside>
