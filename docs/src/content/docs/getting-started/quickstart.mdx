...
- **[Bun](https://bun.sh) runtime 1.0+** for the npm package. Installing via npm or pnpm does not install Bun. Without it, `bb` exits with `Error: This CLI requires the Bun runtime.` The [standalone binary](/getting-started/installation/#standalone-binary) needs no runtime.
