...
|-------|----------|-----------------------|
| Workspace-only | `bb repo list`, `bb repo create`, `bb repo clone`, `bb project *`, `bb snippet *`, `bb workspace view`, `bb search code`, `bb webhook * --scope workspace` | `6002 CONTEXT_WORKSPACE_NOT_FOUND` |
| Repository-scoped | `bb pr *`, `bb pipeline *`, `bb commit *`, `bb status *`, `bb browse`, `bb repo view`, `bb repo delete`, `bb repo default-reviewers *`, `bb webhook *`, `bb branch-restriction *`, `bb deployment *` | `6001 CONTEXT_REPO_NOT_FOUND` |

`bb ssh-key *` and `bb gpg-key *` act on your own account and ignore `-w`/`-r`.
