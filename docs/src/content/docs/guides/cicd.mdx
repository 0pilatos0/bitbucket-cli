...
2. **Install the CLI.** The standalone binary needs no Bun, Node.js or npm, so the job skips the runtime install. Verify it against the release's `SHA256SUMS` before running it:

   ```bash
   base=https://github.com/0pilatos0/bitbucket-cli/releases/latest/download
   curl -fsSLO "$base/bb-linux-x64"
   curl -fsSLO "$base/SHA256SUMS"
   sha256sum --check --ignore-missing SHA256SUMS
   install -D -m 755 bb-linux-x64 "$HOME/.local/bin/bb"
   export PATH="$HOME/.local/bin:$PATH"
   ```

   Replace `latest/download` with `download/v<version>` to pin a release, and `bb-linux-x64` with `bb-linux-arm64` on arm64 agents. See [Installation](/getting-started/installation/#standalone-binary) for the other platforms and for `gh attestation verify`. The binary needs glibc, so on Alpine use the Bun route below.

   To use the npm package instead, install Bun first. The package is Bun-only; it exits immediately under Node.js.
...
Each snippet shows the platform-specific part: how secrets reach the job, and how the CLI gets on `PATH`. The GitHub Actions example uses the standalone binary and the others use Bun; either install from [Quick setup](#quick-setup) works on any of them. The `bb auth login` step is the same everywhere.
...
Appending to `$GITHUB_PATH` puts `bb` on `PATH` for every later step.
