...
<Tabs>
  <TabItem label="Claude Code">
    ...
    **Repos:** `bb repo list`, `bb repo clone workspace/repo`, `bb repo view`; read files without cloning: `bb repo ls [path]`, `bb repo cat <path> --ref <ref>`; release artifacts: `bb repo downloads list/upload/delete`
    ...
    **Raw API:** `bb api <endpoint>` — any Bitbucket 2.0 endpoint not covered above (e.g. `bb api /repositories/{workspace}/{repo}/hooks`; supports `--paginate` and `--jq`)
  ...
  <TabItem label="opencode">
    ...
    **Repositories:**
    ...
    - `bb repo ls [path]` / `bb repo cat <path> --ref <ref>` - Read files without cloning
    - `bb repo downloads list` / `upload <files...>` / `delete <filename> --yes` - Manage download artifacts
    ...
    **Raw API access:**
    - `bb api <endpoint>` - Authenticated call to any Bitbucket 2.0 endpoint (escape hatch for anything without a typed command; e.g. `bb api /repositories/{workspace}/{repo}/hooks`; supports `--paginate` and `--jq`)
...
| `read:repository:bitbucket` | `bb repo list`, `bb repo view`, `bb repo ls/cat`, `bb repo downloads list`, `bb commit list/view`, `bb status list` — browse repos and commits |
| `write:repository:bitbucket` | `bb status set`, `bb repo downloads upload/delete` — set commit build statuses, manage download artifacts |
...
`bb deployment`, `bb project`, `bb snippet`, `bb repo delete`, or adding and
deleting SSH/GPG keys — those return 403 under OAuth.
...
The docs site publishes the [`llms.txt`](https://llmstxt.org/) convention so
...
| URL | Contents |
|-----|----------|
| [`/llms.txt`](https://bitbucket-cli.paulvanderlei.com/llms.txt) | Index pointing to the two content files below |
| [`/llms-full.txt`](https://bitbucket-cli.paulvanderlei.com/llms-full.txt) | Every doc page concatenated as markdown |
